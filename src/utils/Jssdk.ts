...
class Jssdk extends egret.DisplayObjectContainer {
    ...
    public constructor(myScore:number, giftid:number) {
        super();
     ...
     **/
    public init() {
        this.title = this.title ? this.title : "分享标题";
        this.desc = this.desc ? this.desc : "分享描述";
        this.link = this.link ? this.link : "http://XXXX/index";
        this.imgUrl = this.imgUrl ? this.imgUrl : "http://mat1.gtimg.com/xian/cc/h5/zhuaji/icon.jpg";

        this.url = "http://XXXXX/sign?url="+encodeURIComponent(location.href.split('#')[0]);
        this.getSignPackage();
    }
